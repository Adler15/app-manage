...
def run_container():
    try:
        ...
        discovery_uri = request.args.get('discovery_uri')
        config_uri = request.args.get('config_uri')
        logging.info(f'接受的镜像名为：{image_name}')
        # ports = {8090: 8090},
        c = client.containers.run(image=image_name, network='k2data-bcf', name=image_name.split(':')[0],
                                  detach=True,
                                  environment={'SPRING.CLOUD.NACOS.DISCOVERY.URI': discovery_uri,
                                               'SPRING.CLOUD.NACOS.CONFIG.URI': config_uri})
